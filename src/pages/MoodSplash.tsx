...
const MoodSplash = () => {
  ...
  const moodThemes = {
    ...
    neutral: {
      ...
      greeting: "Perfect balance. Let's maintain this peaceful energy.",
      animation: ""
    },
    fearful: {
  ...
  return <div className={`min-h-screen relative overflow-hidden transition-all duration-1000 ${currentTheme ? currentTheme.background : 'bg-gradient-to-br from-purple-100 via-blue-50 to-teal-100 dark:from-purple-900 dark:via-blue-900 dark:to-teal-900'}`}>
      
      {/* Floating Animation Elements */}
      <div className="absolute inset-0 overflow-hidden">
        {selectedMood && <>
            <motion.div className={`absolute top-20 left-20 w-16 h-16 rounded-full blur-xl opacity-30 ${selectedMood === 'happy' ? 'bg-yellow-400' : selectedMood === 'sad' ? 'bg-blue-400' : selectedMood === 'angry' ? 'bg-red-400' : selectedMood === 'neutral' ? 'bg-teal-400' : 'bg-green-400'}`} animate={{
          y: [0, 30, 0],
          x: [0, 20, 0],
          scale: [1, 1.2, 1]
        }} transition={{
          ...
          ease: "easeInOut"
            ...
            <motion.div className={`absolute bottom-32 right-32 w-24 h-24 rounded-full blur-xl opacity-20 ${selectedMood === 'happy' ? 'bg-orange-400' : selectedMood === 'sad' ? 'bg-indigo-400' : selectedMood === 'angry' ? 'bg-orange-400' : selectedMood === 'neutral' ? 'bg-purple-400' : 'bg-teal-400'}`} animate={{
          y: [0, -20, 0],
          x: [0, -15, 0],
          rotate: [0, 180, 360]
        }} transition={{
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4">
        ...
        <motion.div key="mood-selection" initial={{
          opacity: 0,
          y: 30
        }} animate={{
          opacity: 1,
          ...
          duration: 0.8
            ...
            delay: 0.3,
            duration: 1
          }} className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-purple-600 via-blue-600 to-teal-600 bg-clip-text text-transparent mb-6">Moodsify</motion.h1>
            ...
            duration: 0.8
                ...
                {moodOptions.map((mood, index) => <motion.button key={mood.label} onClick={() => handleMoodSelect(mood.label as EmotionType)} whileHover={{
              scale: 1.1
            }} whileTap={{
              scale: 0.95
            }} initial={{
              ...
              delay: 1.1 + index * 0.1,
              duration: 0.5
            }} className="p-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700">
        ...
        // Greeting Phase
        <motion.div key="greeting" initial={{
          opacity: 0,
          scale: 0.8
        }} animate={{
          opacity: 1,
          ...
          type: "spring"
              ...
              <motion.div className={`text-8xl mb-6 ${currentTheme?.animation} inline-block`} animate={selectedMood === 'fearful' ? {
            rotate: [0, 5, -5, 0]
          } : {}} transition={{
            duration: 2,
            repeat: Infinity
            ...
            duration: 0.5
                ...
                <div className="flex justify-center space-x-1">
                  {[...Array(3)].map((_, i) => <motion.div key={i} className={`w-2 h-2 rounded-full ${currentTheme?.accent.replace('text-', 'bg-')}`} animate={{
                opacity: [0.3, 1, 0.3]
              }} transition={{
                duration: 1.5,
                repeat: Infinity,
