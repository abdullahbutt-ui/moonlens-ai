...
const MoodSplash = () => {
  ...
  const moodOptions = [
    ...
    { emoji: "😟", label: "fearful", icon: Zap, name: "Anxious" },
  ];
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden transition-all duration-1000 ${
      currentTheme ? currentTheme.background : 'bg-gradient-to-br from-purple-100 via-blue-50 to-teal-100 dark:from-purple-900 dark:via-blue-900 dark:to-teal-900'
    }`}>
          ...
          <>
            <motion.div
              className={`absolute top-20 left-20 w-16 h-16 rounded-full blur-xl opacity-30 ${
                selectedMood === 'happy' ? 'bg-yellow-400' :
                selectedMood === 'sad' ? 'bg-blue-400' :
                selectedMood === 'angry' ? 'bg-red-400' :
                selectedMood === 'neutral' ? 'bg-teal-400' :
                'bg-green-400'
              }`}
              animate={{ 
                y: [0, 30, 0], 
            ...
            <motion.div
              className={`absolute bottom-32 right-32 w-24 h-24 rounded-full blur-xl opacity-20 ${
                selectedMood === 'happy' ? 'bg-orange-400' :
                selectedMood === 'sad' ? 'bg-indigo-400' :
                selectedMood === 'angry' ? 'bg-orange-400' :
                selectedMood === 'neutral' ? 'bg-purple-400' :
                'bg-teal-400'
              }`}
              animate={{ 
                y: [0, -20, 0], 
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4">
        ...
        <AnimatePresence mode="wait">
          {!selectedMood ? (
            ...
            >
              <motion.div
                className={`text-8xl mb-6 ${currentTheme?.animation} inline-block`}
                animate={selectedMood === 'fearful' ? { rotate: [0, 5, -5, 0] } : {}}
